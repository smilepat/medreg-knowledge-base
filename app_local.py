...
GUIDE = """
### 이 프로그램은 무엇인가요?
의료기기 관련 **법·규정 7개(584개 조항)** 를 통째로 외운 비서입니다.
궁금한 걸 우리말로 물어보면 **"어느 법 몇 조에 그렇게 적혀 있는지"** 를
**출처 링크·시행일과 함께** 찾아줍니다. 설치·로그인·인터넷 연결이 없어도 됩니다.

> ⚠️ 모든 결과는 **초안(참고용)** 입니다. 중요한 판단은 꼭 **출처 원문을 직접 확인**하세요.

---
### 📖 규정 근거찾기 — "이거 무슨 법 몇 조야?"
1. 위쪽 칸에 궁금한 내용을 한 문장으로 적습니다.
2. **[근거 찾기]** 버튼을 누릅니다.
3. 관련 조항이 **출처 링크와 함께** 나옵니다.
   - 예) `의료기기 제조업 허가는 누구에게 받나?`
   - 찾지 못하면 **"확인 불가"** 라고 솔직히 말합니다(지어내지 않음).

### ✅ 문서 점검 — "내가 쓴 문장이 규정에 맞나?"
1. 점검할 문장을 **한 줄에 하나씩** 붙여넣습니다.
2. **[점검하기]** 버튼을 누릅니다.
3. 문장마다 **✅ 근거있음 / ✅ 조문 존재 / ⚠️ 저신뢰 / ⚠️ 근거없음** 으로 표시됩니다.
   - 한 번에 수십 줄 정도가 적당합니다(많으면 나눠서).

### 📋 입찰 매트릭스 — "입찰 요건을 우리가 충족하나?"
1. 왼쪽에 **입찰 요구사항** 을 한 줄씩 붙여넣습니다.
2. (선택) 오른쪽에 **우리 회사 정보** 를 적으면 충족 여부도 대략 비교해 줍니다.
3. **[매트릭스 생성]** → 요구사항별 유형·규정근거·충족(키워드 비교) 표가 나옵니다.

---
### 꼭 알아두기
- **"확인 불가 / 근거없음"은 정상입니다** — 모르면 모른다고 답하도록 만들었습니다.
- 이 버전은 **키워드(낱말) 검색** 으로 빠르게 1차로 찾아주는 간편판입니다.
  뜻이 같지만 낱말이 다른 경우(예: '제조허가' ↔ '만들 권리')는 놓칠 수 있어요.
  → **법령 용어 그대로** 넣으면 더 잘 찾습니다.
- 왼쪽 사이드바에서 **"현행 규정만 사용"** 을 켜고 끌 수 있습니다(보통 켜둠).
- 결과의 **출처 링크를 눌러 원문을 꼭 확인** 하세요. 최종 책임은 담당자에게 있습니다.
"""
...
# 홈화면 상단: 사용 가이드 버튼(초보자용 팝업)
_c1, _c2 = st.columns([1, 4])
with _c1:
    if st.button("📖 사용 가이드", use_container_width=True, type="primary"):
        _show_guide()
with _c2:
    st.caption("처음이신가요? 왼쪽 **📖 사용 가이드** 버튼을 눌러 사용법을 확인하세요.")
